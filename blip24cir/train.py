...
def main():
    ...
    optimizer = optim.AdamW(
        [{'params': filter(lambda p: p.requires_grad, model.blip_model.parameters()), 'lr': args.learning_rate,
            ...
            for idx, batch_data in enumerate(relative_train_loader):
                ...
                with autocast():
                    loss_dict = model.forward(captions, indexs, target_index_all,
                                              reference_index_all)
                loss = loss_dict['loss_qtc']
                loss_avg.update(loss.item())
                loss_log_dict = dict()
                loss_log_dict['loss'] = loss_avg()
                for loss_name in loss_dict:
                    loss_log_dict[loss_name] = '{:05.3f}'.format(loss_dict[loss_name])
                t.set_postfix(OrderedDict(loss_log_dict))
                t.update()
