...
def compute_fiq_val_metrics(relative_val_dataset: CIRDataset, model, index_features: torch.tensor,
    ...
    for i in tqdm(range(N)):
        ...
        sorted_index_name = sorted_index_name.tolist()
        recall_at10_i = 0
        recall_at50_i = 0
        if target_names[i] in sorted_index_name[:10]:
            ...
            recall_at50_i = 1
        k = sorted_index_name.index(target_names[i])
        recall_at10 += recall_at10_i
        recall_at50 += recall_at50_i
        triplet = relative_val_dataset.triplets[i]
        casedata_ls.append({
            "refer_name": triplet['reference_name'],
            "captions": triplet['captions'],
            "target_name": triplet['target_name'],
            "top_k_names": sorted_index_name[:10],
            "k": k,
        })
...
def compute_cirr_val_metrics(relative_val_dataset: CIRDataset, model, index_features: torch.tensor,
    ...
    for i in tqdm(range(N)):
        sorted_index_name = sorted_index_names[i]
        sorted_index_name = sorted_index_name[sorted_index_name != reference_names[i]]
        sorted_index_name = sorted_index_name.tolist()
        k = sorted_index_name.index(target_names[i])
        triplet = relative_val_dataset.triplets[i]
        if target_names[i] in sorted_index_name[:1]:
            recall_at1 += 1
            recall_at5 += 1
            recall_at10 += 1
            recall_at50 += 1
        elif target_names[i] in sorted_index_name[:5]:
            recall_at5 += 1
            recall_at10 += 1
            recall_at50 += 1
        elif target_names[i] in sorted_index_name[:10]:
            recall_at10 += 1
            recall_at50 += 1
        elif target_names[i] in sorted_index_name[:50]:
            recall_at50 += 1
    ...
    recall_at10 /= N
    recall_at50 /= N
    return 0, 0, 0, recall_at1, recall_at5, recall_at10, recall_at50
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model_name", choices=['base', 'full'], default='base')
    ...
    if args.dataset.lower() == 'cirr':
        ...
        print(f"{recall_at10 = }")
        print(f"{recall_at50 = }")
        case_path = f"cases/cirr/{args.model_name}.json"
        with open(case_path, 'w') as f:
            f.write(json.dumps(casedata_ls, ensure_ascii=False))
        ...
        case_path = f"cases/fiq/{args.model_name}.json"
        with open(case_path, 'w') as f:
            f.write(json.dumps(casedata_ls, ensure_ascii=False))
    else:
