...
def generate_cirr_test_submissions(file_name: str, model: torch.nn.Module, preprocess: callable):
    """
   ...
   """
    ...
    index_features, index_features_p, index_names = extract_index_features(classic_test_dataset, model)
    ...
    submissions_folder_path = base_path / "submission" / 'tgcir'
...
def generate_cirr_test_dicts(relative_test_dataset: CIRDataset,
...
def generate_cirr_test_predictions(blip_model: torch.nn.Module,
                                   relative_test_dataset: CIRDataset,
    ...
    for batch_pairs_id, batch_reference_names, captions, batch_group_members in tqdm(
        ...
        with torch.no_grad():
            ...
            batch_predicted_features = blip_model.img_txt_fusion(reference_image_features.to(device), captions)
