...
class CIRDataset(Dataset):
    def __init__(self, data_name, split, mode, preprocess, data_path='./', dress_types=None, val_ret_train=False,
                 fiq_val_type=0, plus=False, use_cc=False):
        ...
        if self.data_name == 'fiq':
            ...
            if self.split == 'train':
                if use_cc:
                    with open(os.path.join(self.caption_path, f'cap.cc.train.json')) as f:
                        self.triplets = json.load(f)
                else:
                    with open(os.path.join(self.caption_path, f'cap.extend_zs.train.json')) as f:
                        self.triplets = json.load(f)
            if not use_cc:
                self.triplets = [
                    {
            ...
            if self.split == 'train':
                if use_cc:
                    with open(os.path.join(self.caption_path, f'cap.rc2.train.cc.json')) as f:
                        self.triplets = json.load(f)
                else:
                    with open(os.path.join(self.caption_path, f'cap.rc2.train.extend_zs.json')) as f:
                        self.triplets = json.load(f)
            if not use_cc:
                self.triplets = [
                    {
                        ...
                        "captions": triplet['caption'] if isinstance(triplet['caption'], list) else [
                            triplet['caption']],
            ...
            if self.split == 'train':
                if not self.use_cc:
                    ...
                    if len(captions) > 1:
                        ...
                        caption = captions[0]
                    if self.use_bank:
                        return caption, index, target_index, reference_index_all, target_index_all
                    else:
