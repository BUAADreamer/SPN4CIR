    ...
    def forward(self, refer_image, text, target_image, indexs, target_indexs, refer_indexs, grad_ckpt=False):
        ...
        if self.use_bank:
            ...
            if grad_ckpt:
                ...
                text_feats = self.encode_text(text)
                refer_image_feats = self.encode_image(refer_image)
                query_feats = self.combining_function(refer_image_feats, text_feats)
                query_feats = F.normalize(query_feats)
                target_image_feats = self.encode_image(target_image)
                target_image_feats = F.normalize(target_image_feats)
            loss = dict()
