...
def main():
    ...
    model = CIRPlus(args.clip_model_name, combiner=args.combiner, tau=args.tau, transform=args.transform,
                    use_bank=args.use_bank, device=device)
    if args.model_path:
    ...
    index_features_list = []
    index_names_list = []
    val_index_features, val_index_names = None, None
    ...
    relative_train_dataset = CIRDataset(args.dataset, 'train', 'relative', preprocess, args.data_path,
                                        args.dress_types, plus=args.plus, use_cc=args.use_cc)
    ...
    for epoch in range(args.num_epochs):
        with tqdm(total=len(relative_train_loader)) as t:
            for idx, batch_data in enumerate(relative_train_loader):
                if args.use_bank:
                    captions, indexs, target_indexs \
                        , reference_index_all, target_index_all = batch_data
                    reference_images, target_images = None, None
                    optimizer.zero_grad()
                    with autocast():
                        loss_dict = model.forward(reference_images, captions, target_images, indexs, target_index_all,
                                                  reference_index_all,
                                                  grad_ckpt=True)
                    loss = loss_dict['bank_loss']
                else:
                    if args.use_cc:
                        reference_images, captions, target_images, indexs, target_indexs, reference_index_all, target_index_all = batch_data
                    else:
                        reference_images, captions, target_images, indexs, target_indexs \
                            , reference_index_all, target_index_all = batch_data
                    reference_images = reference_images.to(device, non_blocking=True)
                    target_images = target_images.to(device, non_blocking=True)
                    optimizer.zero_grad()
                    with autocast():
                        loss_dict = model.forward(reference_images, captions, target_images, indexs, target_indexs,
                                                  refer_indexs=reference_index_all,
                                                  grad_ckpt=True)
                    loss = loss_dict['bbc_loss']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--combiner", default='sum')
    parser.add_argument("--grad_ckpt", action='store_true', help='whether use gradient checkpoint')
